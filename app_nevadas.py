...
st.set_page_config(page_title="Gestor de Rutas Automático", layout="wide")

st.title("⏱️ Gestor de Partes (Km y Tiempo Automáticos)")

# --- 1. DATOS INTEGRADOS ---
DATOS_RUTAS = [
    {'layer': 'R1-1', 'ESTADO': 'CERRADA', 'HIELO': None, 'DISTANCIA': 3.21, 'PASADAS': 2.0, 'RECURSO': None, 'ACTUACION': 'SALERO', 'FUNDENTE': None, 'FECHA': '2025-09-26', 'RUTA': 1, 'TRAMO': '1º', 'TOTAL_KM': 0.0, 'PERSONAL': 'vol morella', 'RECORRIDO': 'CV108 Ballestar a CV107 La Pobla de Benifassar', 'H.INICIO': 1000.0, 'H.FIN': 1300.0, 'TIEMPO': 180.0},
    {'layer': 'R1-2', 'ESTADO': 'CADENAS', 'HIELO': 'HIELO', 'DISTANCIA': 15.73, 'PASADAS': 3.0, 'RECURSO': 'BRP531', 'ACTUACION': 'CUCHILLA', 'FUNDENTE': None, 'FECHA': '2025-09-26', 'RUTA': 1, 'TRAMO': '2º', 'TOTAL_KM': 0.0, 'PERSONAL': 'ubf morella', 'RECORRIDO': 'CV105 cruce CV107 a La Senia', 'H.INICIO': 1130.0, 'H.FIN': 1300.0, 'TIEMPO': 90.0},
...
with st.container(border=True):
    ...
    with st.form("form_ruta_pro"):
        ...
        ca3.text_input("Total Km", value="Auto-calc", disabled=True) # Solo visual
        ...
        # Campo Tiempo bloqueado
        cb4.text_input("Tiempo (min)", value="Auto-calc", disabled=True)
        ...
        submit_btn = st.form_submit_button("➕ Calcular y Guardar", type="primary")
        
        if submit_btn:
            # 1. CÁLCULO KM
            calculo_total_km = val_distancia * val_pasadas
            
            # 2. CÁLCULO TIEMPO (Diferencia de horas)
            # Creamos fechas completas usando el día de hoy para poder restar
            ...
            # Si sale negativo (ej. de 23:00 a 01:00), sumamos 24h (1440 min)
            ...
            nueva_fila = {
                ...
                'H.FIN': str_hfin,
                'TIEMPO': round(minutos_totales) # Guardamos minutos calculados
            }
            
            st.session_state['seleccionados'] = pd.concat([st.session_state['seleccionados'], pd.DataFrame([nueva_fila])], ignore_index=True)
            st.success(f"✅ Guardado: {minutos_totales:.0f} min | {calculo_total_km:.2f} km")
...
if not df_resultados.empty:
    ...
    with col_d1:
        st.download_button(
            label="💾 Descargar CSV",
            data=csv_bytes,
            file_name="parte_rutas_final.csv",
            mime="text/csv",
        )
    with col_d2:
    ...
    st.info("Rellena el formulario y pulsa 'Calcular y Guardar'.")
